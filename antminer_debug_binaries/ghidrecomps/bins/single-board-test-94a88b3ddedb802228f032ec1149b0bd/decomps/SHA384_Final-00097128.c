
int SHA384_Final(uchar *md,SHA512_CTX *c)

{
  uint uVar1;
  uint uVar2;
  size_t __n;
  undefined4 uVar3;
  undefined4 uVar4;
  undefined4 uVar5;
  undefined4 uVar6;
  
  uVar2 = c->num;
  uVar1 = uVar2 + 1;
  *(undefined *)((int)c->h + uVar2 + 0x50) = 0x80;
  if (uVar1 < 0x71) {
    __n = 0x6f - uVar2;
  }
  else {
    memset((void *)((int)c->h + uVar2 + 0x51),0,0x7f - uVar2);
    sha512_block_data_order(c,&c->u,1);
    __n = 0x70;
    uVar1 = 0;
  }
  memset((void *)((int)c->h + uVar1 + 0x50),0,__n);
  uVar6 = *(undefined4 *)&c->Nl;
  uVar5 = *(undefined4 *)((int)&c->Nl + 4);
  uVar4 = *(undefined4 *)&c->Nh;
  uVar3 = *(undefined4 *)((int)&c->Nh + 4);
  (c->u).p[0x7f] = (uchar)uVar6;
  (c->u).p[0x7e] = (uchar)((uint)uVar6 >> 8);
  (c->u).p[0x7d] = (uchar)((uint)uVar6 >> 0x10);
  (c->u).p[0x7c] = (uchar)((uint)uVar6 >> 0x18);
  (c->u).p[0x7b] = (uchar)uVar5;
  (c->u).p[0x7a] = (uchar)((uint)uVar5 >> 8);
  (c->u).p[0x79] = (uchar)((uint)uVar5 >> 0x10);
  (c->u).p[0x78] = (uchar)((uint)uVar5 >> 0x18);
  (c->u).p[0x77] = (uchar)uVar4;
  (c->u).p[0x76] = (uchar)((uint)uVar4 >> 8);
  (c->u).p[0x75] = (uchar)((uint)uVar4 >> 0x10);
  (c->u).p[0x74] = (uchar)((uint)uVar4 >> 0x18);
  (c->u).p[0x73] = (uchar)uVar3;
  (c->u).p[0x72] = (uchar)((uint)uVar3 >> 8);
  (c->u).p[0x71] = (uchar)((uint)uVar3 >> 0x10);
  (c->u).p[0x70] = (uchar)((uint)uVar3 >> 0x18);
  sha512_block_data_order(c,&c->u,1);
  if (md != (uchar *)0x0) {
    if (c->md_len == 0x30) {
      uVar3 = *(undefined4 *)c->h;
      uVar4 = *(undefined4 *)((int)c->h + 4);
      md[7] = (uchar)uVar3;
      md[4] = (uchar)((uint)uVar3 >> 0x18);
      *md = (uchar)((uint)uVar4 >> 0x18);
      md[1] = (uchar)((uint)uVar4 >> 0x10);
      md[2] = (uchar)((uint)uVar4 >> 8);
      md[5] = (uchar)((uint)uVar3 >> 0x10);
      md[3] = (uchar)uVar4;
      md[6] = (uchar)((uint)uVar3 >> 8);
      uVar4 = *(undefined4 *)((int)c->h + 0xc);
      uVar3 = *(undefined4 *)(c->h + 1);
      md[0xb] = (uchar)uVar4;
      md[0xf] = (uchar)uVar3;
      md[0xc] = (uchar)((uint)uVar3 >> 0x18);
      md[8] = (uchar)((uint)uVar4 >> 0x18);
      md[9] = (uchar)((uint)uVar4 >> 0x10);
      md[10] = (uchar)((uint)uVar4 >> 8);
      md[0xd] = (uchar)((uint)uVar3 >> 0x10);
      md[0xe] = (uchar)((uint)uVar3 >> 8);
      uVar4 = *(undefined4 *)((int)c->h + 0x14);
      uVar3 = *(undefined4 *)(c->h + 2);
      md[0x13] = (uchar)uVar4;
      md[0x17] = (uchar)uVar3;
      md[0x14] = (uchar)((uint)uVar3 >> 0x18);
      md[0x10] = (uchar)((uint)uVar4 >> 0x18);
      md[0x11] = (uchar)((uint)uVar4 >> 0x10);
      md[0x12] = (uchar)((uint)uVar4 >> 8);
      md[0x15] = (uchar)((uint)uVar3 >> 0x10);
      md[0x16] = (uchar)((uint)uVar3 >> 8);
      uVar3 = *(undefined4 *)((int)c->h + 0x1c);
      uVar4 = *(undefined4 *)(c->h + 3);
      md[0x18] = (uchar)((uint)uVar3 >> 0x18);
      md[0x19] = (uchar)((uint)uVar3 >> 0x10);
      md[0x1a] = (uchar)((uint)uVar3 >> 8);
      md[0x1c] = (uchar)((uint)uVar4 >> 0x18);
      md[0x1d] = (uchar)((uint)uVar4 >> 0x10);
      md[0x1e] = (uchar)((uint)uVar4 >> 8);
      md[0x1b] = (uchar)uVar3;
      md[0x1f] = (uchar)uVar4;
      uVar4 = *(undefined4 *)((int)c->h + 0x24);
      uVar3 = *(undefined4 *)(c->h + 4);
      md[0x23] = (uchar)uVar4;
      md[0x27] = (uchar)uVar3;
      md[0x20] = (uchar)((uint)uVar4 >> 0x18);
      md[0x21] = (uchar)((uint)uVar4 >> 0x10);
      md[0x22] = (uchar)((uint)uVar4 >> 8);
      md[0x25] = (uchar)((uint)uVar3 >> 0x10);
      md[0x24] = (uchar)((uint)uVar3 >> 0x18);
      md[0x26] = (uchar)((uint)uVar3 >> 8);
      uVar3 = *(undefined4 *)(c->h + 5);
      uVar4 = *(undefined4 *)((int)c->h + 0x2c);
      md[0x2f] = (uchar)uVar3;
      md[0x2c] = (uchar)((uint)uVar3 >> 0x18);
      md[0x2b] = (uchar)uVar4;
      md[0x28] = (uchar)((uint)uVar4 >> 0x18);
      md[0x29] = (uchar)((uint)uVar4 >> 0x10);
      md[0x2a] = (uchar)((uint)uVar4 >> 8);
      md[0x2d] = (uchar)((uint)uVar3 >> 0x10);
      md[0x2e] = (uchar)((uint)uVar3 >> 8);
      return 1;
    }
    if (c->md_len == 0x40) {
      uVar3 = *(undefined4 *)c->h;
      uVar4 = *(undefined4 *)((int)c->h + 4);
      md[7] = (uchar)uVar3;
      md[4] = (uchar)((uint)uVar3 >> 0x18);
      *md = (uchar)((uint)uVar4 >> 0x18);
      md[1] = (uchar)((uint)uVar4 >> 0x10);
      md[2] = (uchar)((uint)uVar4 >> 8);
      md[5] = (uchar)((uint)uVar3 >> 0x10);
      md[3] = (uchar)uVar4;
      md[6] = (uchar)((uint)uVar3 >> 8);
      uVar4 = *(undefined4 *)((int)c->h + 0xc);
      uVar3 = *(undefined4 *)(c->h + 1);
      md[0xb] = (uchar)uVar4;
      md[0xf] = (uchar)uVar3;
      md[0xc] = (uchar)((uint)uVar3 >> 0x18);
      md[8] = (uchar)((uint)uVar4 >> 0x18);
      md[9] = (uchar)((uint)uVar4 >> 0x10);
      md[10] = (uchar)((uint)uVar4 >> 8);
      md[0xd] = (uchar)((uint)uVar3 >> 0x10);
      md[0xe] = (uchar)((uint)uVar3 >> 8);
      uVar4 = *(undefined4 *)((int)c->h + 0x14);
      uVar3 = *(undefined4 *)(c->h + 2);
      md[0x13] = (uchar)uVar4;
      md[0x17] = (uchar)uVar3;
      md[0x14] = (uchar)((uint)uVar3 >> 0x18);
      md[0x10] = (uchar)((uint)uVar4 >> 0x18);
      md[0x11] = (uchar)((uint)uVar4 >> 0x10);
      md[0x12] = (uchar)((uint)uVar4 >> 8);
      md[0x15] = (uchar)((uint)uVar3 >> 0x10);
      md[0x16] = (uchar)((uint)uVar3 >> 8);
      uVar3 = *(undefined4 *)((int)c->h + 0x1c);
      uVar4 = *(undefined4 *)(c->h + 3);
      md[0x18] = (uchar)((uint)uVar3 >> 0x18);
      md[0x19] = (uchar)((uint)uVar3 >> 0x10);
      md[0x1a] = (uchar)((uint)uVar3 >> 8);
      md[0x1c] = (uchar)((uint)uVar4 >> 0x18);
      md[0x1d] = (uchar)((uint)uVar4 >> 0x10);
      md[0x1e] = (uchar)((uint)uVar4 >> 8);
      md[0x1b] = (uchar)uVar3;
      md[0x1f] = (uchar)uVar4;
      uVar4 = *(undefined4 *)((int)c->h + 0x24);
      uVar3 = *(undefined4 *)(c->h + 4);
      md[0x23] = (uchar)uVar4;
      md[0x27] = (uchar)uVar3;
      md[0x24] = (uchar)((uint)uVar3 >> 0x18);
      md[0x20] = (uchar)((uint)uVar4 >> 0x18);
      md[0x21] = (uchar)((uint)uVar4 >> 0x10);
      md[0x22] = (uchar)((uint)uVar4 >> 8);
      md[0x25] = (uchar)((uint)uVar3 >> 0x10);
      md[0x26] = (uchar)((uint)uVar3 >> 8);
      uVar4 = *(undefined4 *)((int)c->h + 0x2c);
      uVar3 = *(undefined4 *)(c->h + 5);
      md[0x2b] = (uchar)uVar4;
      md[0x2f] = (uchar)uVar3;
      md[0x2c] = (uchar)((uint)uVar3 >> 0x18);
      md[0x28] = (uchar)((uint)uVar4 >> 0x18);
      md[0x29] = (uchar)((uint)uVar4 >> 0x10);
      md[0x2a] = (uchar)((uint)uVar4 >> 8);
      md[0x2d] = (uchar)((uint)uVar3 >> 0x10);
      md[0x2e] = (uchar)((uint)uVar3 >> 8);
      uVar4 = *(undefined4 *)(c->h + 6);
      uVar3 = *(undefined4 *)((int)c->h + 0x34);
      md[0x33] = (uchar)uVar3;
      md[0x31] = (uchar)((uint)uVar3 >> 0x10);
      md[0x32] = (uchar)((uint)uVar3 >> 8);
      md[0x34] = (uchar)((uint)uVar4 >> 0x18);
      md[0x30] = (uchar)((uint)uVar3 >> 0x18);
      md[0x35] = (uchar)((uint)uVar4 >> 0x10);
      md[0x36] = (uchar)((uint)uVar4 >> 8);
      md[0x37] = (uchar)uVar4;
      uVar3 = *(undefined4 *)(c->h + 7);
      uVar4 = *(undefined4 *)((int)c->h + 0x3c);
      md[0x3f] = (uchar)uVar3;
      md[0x3c] = (uchar)((uint)uVar3 >> 0x18);
      md[0x3b] = (uchar)uVar4;
      md[0x38] = (uchar)((uint)uVar4 >> 0x18);
      md[0x39] = (uchar)((uint)uVar4 >> 0x10);
      md[0x3a] = (uchar)((uint)uVar4 >> 8);
      md[0x3d] = (uchar)((uint)uVar3 >> 0x10);
      md[0x3e] = (uchar)((uint)uVar3 >> 8);
      return 1;
    }
  }
  return 0;
}

