...
void * BHB91603_show_status_func(void *arg)

{
  ...
  printf("\n--- %s: which_chain = %d, which_i2c = %d\n","BHB91603_show_status_func",uVar6,
         (uint)*(byte *)((int)arg + 1));
  __s = puVar1 + -8;
  time_counter = 0;
  do {
    memset(__s,0x20,0x40);
    fan_control(*(uchar *)(DAT_0001663c + 0x328));
    ...
    sprintf(DAT_00016640,"   nonce=%d",gValid_Nonce_Num[uVar6]);
    ...
    if (Conf.TempSensor1 == 0) {
      ...
      *(undefined2 *)(puVar1 + 4) = uVar5;
      ...
      sprintf((char *)DAT_00016638,"   T1 %d `C",Sensor1_temp);
    }
    else if (Conf.TempSensor3 == 0) {
      sprintf((char *)DAT_00016638,"   T1 %d `C",Sensor1_temp);
      sprintf(DAT_00016644,"   T2 %d `C",Sensor2_temp);
    }
    else if (Conf.TempSensor4 == 0) {
      sprintf((char *)DAT_00016638," T1:%d   T2:%d",Sensor1_temp,Sensor2_temp);
      sprintf(DAT_00016644," T3:%d",Sensor3_temp);
    }
    else {
      sprintf((char *)DAT_00016638," T1:%d   T2:%d",Sensor1_temp,Sensor2_temp);
      sprintf(DAT_00016644," T3:%d   T4:%d",Sensor3_temp,Sensor4_temp);
    }
