...
uint check_BM1397_asic_reg(uchar which_chain,uchar chip_addr,uchar reg,_Bool mode)

{
  ...
  BM1397_get_status(which_chain,chip_addr,reg,mode);
  if (reg == '\0') {
    *(undefined *)(DAT_000268c0 + uVar3 + 0x30c) = 0;
  }
  iVar7 = 0;
  local_38 = 0;
  do {
    ...
    do {
      while( true ) {
        ...
        if (uVar2 == reg_value_buf->reg_buffer[uVar5].crc) {
          if (reg == '\0') {
            if ((reg_buf[0] == '\x13') && (reg_buf[1] == 0x97)) {
        ...
        uVar8 = uVar8 + 1;
        pthread_mutex_unlock((pthread_mutex_t *)&reg_mutex);
        if (uVar9 == uVar8) goto LAB_0002681e;
      }
      puVar1 = &reg_value_buf->reg_value_num;
