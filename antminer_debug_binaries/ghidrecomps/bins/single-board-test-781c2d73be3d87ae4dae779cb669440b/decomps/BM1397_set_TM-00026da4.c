...
void BM1397_set_TM(uchar which_chain,uchar asic_addr,uint tm,_Bool mode)

{
  ...
  buf[2] = asic_addr;
  buf[3] = '\x14';
  buf._4_2_ = CONCAT11(bit_swap_table[(tm << 8) >> 0x18],bit_swap_table[tm >> 0x18]);
  buf._4_3_ = CONCAT12(bit_swap_table[(tm << 0x10) >> 0x18],buf._4_2_);
  buf[7] = bit_swap_table[tm & 0xff];
  buf[8] = CRC5(buf,'@');
