...
void * BHB91603_send_func(void *arg)

{
  ...
  printf("\n--- %s: which_chain = %d, which_i2c = %d\n",DAT_00016998,uVar11,(uint)bVar1);
  ...
  if (BHB91603_ASIC_NUMBER == 0) {
    ...
    do {
      ...
      local_9c = local_9c + 1;
    } while (local_9c < BHB91603_ASIC_NUMBER);
    uVar3 = gSend_Work_Num[uVar11];
    uVar7 = uVar7 * BHB91603_ASIC_NUMBER * 0x100;
    if (uVar3 <= uVar7 && uVar7 - uVar3 != 0) {
      do {
        printf("\n\n---%s: Chain%d send work num : %d, less than %d\n",uVar2,uVar11,uVar3,uVar7);
        uVar3 = gSend_Work_Num[uVar11];
        uVar7 = BHB91603_ASIC_NUMBER * Conf.pattern_number * 0x100;
      } while (uVar3 < uVar7);
    }
  }
  printf("\n\n---%s: Chain%d send work num : %d\n",DAT_00016998,uVar11,uVar3);
