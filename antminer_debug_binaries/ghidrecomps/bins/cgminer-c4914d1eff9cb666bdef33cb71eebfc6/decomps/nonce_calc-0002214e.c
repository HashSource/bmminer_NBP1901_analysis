...

{
  ...
  if ((((chain < 0x10) && (uVar1 < 0x3c)) && (uVar3 < 0xd0)) && (buf != 0)) {
    calc_nonce_info.chain_asic_core_nonce_num[chain][uVar1][uVar3] =
         calc_nonce_info.chain_asic_core_nonce_num[chain][uVar1][uVar3] + 1;
  }
  else {
    if (4 < log_level) {
      ...
      if (pFVar2 != (FILE *)0x0) {
        fprintf(pFVar2,"%s:%d:%s: buf [%x] is error!\n","driver-btm-soc.c",0x1050,"nonce_calc",buf);
      }
      fclose(pFVar2);
    }
    if (4 < log_level) {
      ...
      pFVar2 = fopen(log_file,"a+");
      if (pFVar2 != (FILE *)0x0) {
        fprintf(pFVar2,"%s:%d:%s: chain = %d,chip = %d,core = %d\n","driver-btm-soc.c",0x1051,
                "nonce_calc",chain,uVar1,uVar3);
      }
      fclose(pFVar2);
    }
  }
