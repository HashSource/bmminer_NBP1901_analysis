...

{
  ...
  diff_cleared = 0.0;
  cleared = 0;
  _mutex_lock(&sshare_lock,"cgminer.c","clear_stratum_shares",0x1cab);
  ...
  while (sshare != (stratum_share *)0x0) {
    if (pool == sshare->work->pool) {
      ...
      _free_work(&sshare->work,"cgminer.c","clear_stratum_shares",0x1cb3);
  ...
  _mutex_unlock(&sshare_lock,"cgminer.c","clear_stratum_shares",0x1cb9);
  if (cleared != 0) {
