...

{
  ...
  int iVar4;
  undefined4 uVar5;
  undefined4 in_stack_fffff750;
  char *pcVar6;
  undefined4 in_stack_fffff754;
  undefined4 uVar7;
  work *work_local;
  pool *pool_local;
  char tmp42 [2048];
  ...
  _cg_wlock(&pool->data_lock,"cgminer.c","gen_stratum_work",0x2083);
  nonce2le = __uint64_identity(CONCAT44(in_stack_fffff754,in_stack_fffff750));
  _cg_memcpy(pool->coinbase + pool->nonce2_offset,&nonce2le,pool->n2size,"cgminer.c",
             "gen_stratum_work",0x2088);
  ...
  *(int *)((int)&work->nonce2 + 4) = iVar4;
  ...
  _cg_dwlock(&pool->data_lock,"cgminer.c","gen_stratum_work",0x208e);
  gen_hash(pool->coinbase,merkle_root,pool->coinbase_len);
  _cg_memcpy(merkle_sha,merkle_root,0x20,"cgminer.c","gen_stratum_work",0x2092);
  for (i = 0; i < pool->merkles; i = i + 1) {
    ...
    _cg_memcpy(merkle_sha,merkle_root,0x20,"cgminer.c","gen_stratum_work",0x2098);
  }
  flip32(merkle_root,merkle_sha);
  _cg_memcpy(work,pool->header_bin,0x70,"cgminer.c","gen_stratum_work",0x20a1);
  uVar7 = 0x20a2;
  pcVar6 = "gen_stratum_work";
  _cg_memcpy(work->data + 0x24,merkle_root,0x20,"cgminer.c","gen_stratum_work",0x20a2);
  ...
  *(undefined4 *)((int)&work->sdiff + 4) = uVar5;
  ...
  work->ntime = pcVar1;
  _cg_runlock(&pool->data_lock,"cgminer.c","gen_stratum_work",0x20ad);
  if (opt_debug != false) {
