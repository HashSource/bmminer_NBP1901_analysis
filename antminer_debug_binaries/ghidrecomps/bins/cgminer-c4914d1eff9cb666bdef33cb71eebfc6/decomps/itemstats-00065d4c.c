...

{
  ...
  if (displayed_hash_rate[0] == '\0') {
    sprintf(displayed_hash_rate,"%.2f",0,0);
  }
  root = api_add_string(root,"GHS 5s",displayed_hash_rate,false);
  root = api_add_mhs(root,"GHS av",&ghs,false);
  root = api_add_string(root,"Voltage",displayed_working_voltage,false);
  root = api_add_string(root,"Mode",displayed_mode,false);
  if (extra != (api_data *)0x0) {
    root = api_add_extra(root,extra);
  }
  print_data(io_data_local,root,isjson,true);
  return i_local + 1;
}
