  ...
  if (_Var1) {
    ...
    if (iVar5 < 0) {
      ...
      if (3 < log_level) {
        ...
        pFVar2 = fopen(log_file,"a+");
        if (pFVar2 != (FILE *)0x0) {
          fprintf(pFVar2,"%s:%d:%s: sweep freq for mode 2\n","freq_scan.c",0xe78,
                  "sweep_freq_by_column");
        }
        fclose(pFVar2);
      }
      iVar5 = sweep_freq_for_mode(MODE2);
      check_chain();
      scan_freq_stop_hash_board();
      sleep(0x1e);
      if ((iVar5 == 0) && (iVar5 = eeprom_date_check(), iVar5 == 0)) {
        eeprom_dump();
        scan_freq_done_mark();
        ...
        if (3 < log_level) {
          ...
          if (pFVar2 != (FILE *)0x0) {
            fprintf(pFVar2,"%s:%d:%s: total cost time %ld s \n","freq_scan.c",0xe90,
                    "sweep_freq_by_column",tVar4 - tVar3);
          }
          fclose(pFVar2);
        }
        iVar5 = 0;
