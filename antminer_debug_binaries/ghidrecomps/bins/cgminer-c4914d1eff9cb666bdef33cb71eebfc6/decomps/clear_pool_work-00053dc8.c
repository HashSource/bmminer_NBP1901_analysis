...

{
  ...
  cleared = 0;
  _mutex_lock(stgd_lock,"cgminer.c","clear_pool_work",0x1cca);
  work = staged_work;
  ...
  while (work != (work *)0x0) {
    if (pool == work->pool) {
      ...
      _free_work(&work,"cgminer.c","clear_pool_work",0x1cd0);
      cleared = cleared + 1;
    }
    work = tmp;
    if (tmp == (work *)0x0) {
  ...
  _mutex_unlock(stgd_lock,"cgminer.c","clear_pool_work",0x1cd4);
  if ((cleared != 0) &&
