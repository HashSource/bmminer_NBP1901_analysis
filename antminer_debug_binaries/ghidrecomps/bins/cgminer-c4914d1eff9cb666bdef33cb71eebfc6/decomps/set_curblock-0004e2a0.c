...

{
  ...
  _cg_wlock(&ch_lock,"cgminer.c","set_curblock",0x157a);
  cgtime(&block_timeval);
  strcpy(current_hash,hexstr);
  _cg_memcpy(current_block,bedata,0x20,"cgminer.c","set_curblock",0x157d);
  get_timestamp(blocktime,0x20,&block_timeval);
  _cg_wunlock(&ch_lock,"cgminer.c","set_curblock",0x157f);
  for (ofs = 0; (ofs < 0x39 && (current_hash[ofs] == (char)DAT_0008c974)); ofs = ofs + 1) {
  }
