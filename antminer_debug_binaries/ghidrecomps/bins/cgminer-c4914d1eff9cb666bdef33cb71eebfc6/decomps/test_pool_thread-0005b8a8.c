...

{
  ...
  while( true ) {
    ...
    if (_Var1) break;
    pool_died((pool *)arg);
    if (*(char *)((int)arg + 0x6b) == '\x01') goto LAB_0005ba2a;
    sleep(0x1e);
  }
  pool_tclear((pool *)arg,(_Bool *)((int)arg + 0x61));
  bVar2 = false;
  _cg_wlock(&control_lock,"cgminer.c","test_pool_thread",0x29d7);
  ...
  _cg_wunlock(&control_lock,"cgminer.c","test_pool_thread",0x29e5);
