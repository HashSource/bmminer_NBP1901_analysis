...

{
  ...
  _cg_wlock(&control_lock,"cgminer.c","switch_pools",0x1468);
  pool_00 = currentpool;
  ...
  currentpool = ppVar2;
  _cg_wunlock(&control_lock,"cgminer.c","switch_pools",0x14b9);
  ...
  _mutex_lock(&lp_lock,"cgminer.c","switch_pools",0x14c1);
  pthread_cond_broadcast((pthread_cond_t *)&lp_cond);
  _mutex_unlock(&lp_lock,"cgminer.c","switch_pools",0x14c3);
  return;
}
