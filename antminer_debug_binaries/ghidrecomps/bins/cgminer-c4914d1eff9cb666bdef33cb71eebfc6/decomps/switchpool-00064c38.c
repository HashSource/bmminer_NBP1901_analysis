...

{
  ...
  if (total_pools == 0) {
    ...
    paramid = atoi(param);
    _cg_rlock(&control_lock,"api-btm.c","switchpool",0xb51);
    if ((paramid < 0) || (total_pools <= paramid)) {
      _cg_runlock(&control_lock,"api-btm.c","switchpool",0xb54);
      message(io_data,0x1a,paramid,(char *)0x0,isjson);
    }
    else {
      selected = pools[paramid];
      selected->enabled = POOL_ENABLED;
      _cg_runlock(&control_lock,"api-btm.c","switchpool",0xb5b);
