...

{
  ...
  for (i = 0; i < 0x10; i = i + 1) {
    if (dev->chain_exist[i] == 1) {
      dVar2 = get_pic_an_voltage2((uint8_t)i);
      volt = volt + dVar2;
  ...
  if (total_chain == 0) {
    if (3 < log_level) {
      ...
      pFVar1 = fopen(power_log_file,"a+");
      if (pFVar1 != (FILE *)0x0) {
        fprintf(pFVar1,"%s:%d:%s: error: total_chain get 0 !!","power.c",0x443,"get_average_voltage"
               );
      }
      fclose(pFVar1);
    }
    ...
    if (3 < log_level) {
      ...
      pFVar1 = fopen(power_log_file,"a+");
      if (pFVar1 != (FILE *)0x0) {
        fprintf(pFVar1,"%s:%d:%s: average voltage is: %f \n","power.c",0x448,"get_average_voltage");
      }
      fclose(pFVar1);
    }
