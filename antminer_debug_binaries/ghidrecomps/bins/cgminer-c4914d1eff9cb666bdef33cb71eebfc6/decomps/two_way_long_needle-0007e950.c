...

{
  ...
  if (iVar1 == 0) {
    ...
    while (j <= haystack_len - needle_len) {
      shift = shift_table[haystack[needle_len + j + -1]];
      if (shift == 0) {
        i = memory;
        if (memory < suffix) {
          i = suffix;
        }
        for (; (i < needle_len - 1 && (needle[i] == haystack[j + i])); i = i + 1) {
        }
        if (i < needle_len - 1) {
          j = j + (i - suffix) + 1;
          memory = 0;
          ...
          for (i = suffix - 1; (memory < i + 1 && (needle[i] == haystack[j + i])); i = i - 1) {
          }
          if (i + 1 < memory + 1) {
            return haystack + j;
          }
    ...
    j = 0;
    while (j <= haystack_len - needle_len) {
      shift_1 = shift_table[haystack[needle_len + j + -1]];
      if (shift_1 == 0) {
        for (i = suffix; (i < needle_len - 1 && (needle[i] == haystack[j + i])); i = i + 1) {
        }
        if (i < needle_len - 1) {
          j = j + (i - suffix) + 1;
        }
        else {
          for (i = suffix - 1; (i != 0xffffffff && (needle[i] == haystack[j + i])); i = i - 1) {
          }
          if (i == 0xffffffff) {
            return haystack + j;
          }
