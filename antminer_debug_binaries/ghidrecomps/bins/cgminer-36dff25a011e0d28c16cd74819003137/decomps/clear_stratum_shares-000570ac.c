...

{
  ...
  diff_cleared = 0.0;
  cleared = 0;
  _mutex_lock(&sshare_lock,"cgminer.c","clear_stratum_shares",0x1ca8);
  ...
  while (sshare != (stratum_share *)0x0) {
    if (pool == sshare->work->pool) {
      if (((sshare->hh).prev == (void *)0x0) && ((sshare->hh).next == (void *)0x0)) {
        ...
        if (sshare == (stratum_share *)((stratum_shares->hh).tbl)->buckets[uVar2].hh_head) {
      ...
      _free_work(&sshare->work,"cgminer.c","clear_stratum_shares",0x1cb0);
  ...
  _mutex_unlock(&sshare_lock,"cgminer.c","clear_stratum_shares",0x1cb6);
  if (cleared != 0) {
    ...
    bVar4 = CARRY4(cleared,(uint)total_stale);
    total_stale._0_4_ = cleared + (uint)total_stale;
    total_stale._4_4_ = (cleared >> 0x1f) + total_stale._4_4_ + (uint)bVar4;
