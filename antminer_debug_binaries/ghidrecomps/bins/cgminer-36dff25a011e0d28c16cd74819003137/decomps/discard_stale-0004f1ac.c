...

{
  ...
  stale = 0;
  _mutex_lock(stgd_lock,"cgminer.c","discard_stale",0x14ec);
  work = staged_work;
  ...
  while (work != (work *)0x0) {
    ...
    if (_Var1) {
      ...
      _discard_work(&work,"cgminer.c","discard_stale",0x14f3);
      stale = stale + 1;
    }
    work = tmp;
    if (tmp == (work *)0x0) {
  ...
  _mutex_unlock(stgd_lock,"cgminer.c","discard_stale",0x14f9);
