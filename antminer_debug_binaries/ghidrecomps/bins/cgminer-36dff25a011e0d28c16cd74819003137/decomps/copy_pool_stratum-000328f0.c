...

{
  ...
  if ((pool->swork).job_id != (char *)0x0) {
    ...
    _cg_wlock(&pool_stratum->data_lock,"driver-btm-soc.c","copy_pool_stratum",0x206b);
    free((pool_stratum->swork).job_id);
    free(pool_stratum->nonce1);
    free(pool_stratum->coinbase);
    puVar4 = (uchar *)_cgcalloc(__n,1,"driver-btm-soc.c","copy_pool_stratum",0x2070);
    ...
    *(undefined4 *)((int)&pool_stratum->sdiff + 4) = uVar8;
    ...
    _cg_wunlock(&pool_stratum->data_lock,"driver-btm-soc.c","copy_pool_stratum",0x208b);
  }
  return;
}
