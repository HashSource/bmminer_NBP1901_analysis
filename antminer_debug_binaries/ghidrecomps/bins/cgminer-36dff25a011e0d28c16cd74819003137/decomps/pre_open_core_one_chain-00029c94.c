...
void pre_open_core_one_chain(uint32_t num,uint8_t chain)

{
  ...
  if (dev->chain_exist[chain] == 1) {
    for (loop = 0; loop < num; loop = loop + 1) {
      enable_core_clock_BM1391(loop,'\x01',chain);
      enable_core_clock_BM1391(loop + 0x54,'\x01',chain);
      enable_core_clock_BM1391(loop + 0xa8,'\x01',chain);
      enable_core_clock_BM1391(loop + 0xfc,'\x01',chain);
