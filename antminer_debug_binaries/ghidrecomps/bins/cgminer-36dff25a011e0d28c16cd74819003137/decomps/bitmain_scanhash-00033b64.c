...

{
  ...
  _cg_rlock(&info->update_lock,"driver-btm-soc.c","bitmain_scanhash",0x222e);
  do {
    while( true ) {
      while( true ) {
        while( true ) {
          if (nonce_read_out.nonce_num == 0) {
            ...
            if ((((uint)h | h._4_4_) != 0) && (4 < log_level)) {
              ...
              if (pFile_6 != (FILE *)0x0) {
                fprintf((FILE *)pFile_6,"%s:%d:%s: hashes %llu ...\n","driver-btm-soc.c",0x228c,
                        "bitmain_scanhash");
              }
              fclose((FILE *)pFile_6);
            }
            h = CONCAT44(((uint)h & 0xfffffffe) +
                         ((uint)((uint)h * -0x80000000 < (uint)h) * -2 - h._4_4_) +
          ...
          if (4 < log_level) {
            ...
            if (pFile != (FILE *)0x0) {
              fprintf((FILE *)pFile,
                      ...
                      ,"driver-btm-soc.c",0x223d,"bitmain_scanhash",chain_id,job_id,work_id,
      ...
      if (4 < log_level) {
        ...
        if (pFile_4 != (FILE *)0x0) {
          fprintf((FILE *)pFile_4,"%s:%d:%s: given_id:%d job_id:%d switch:%d  ...\n",
                  "driver-btm-soc.c",0x226c,"bitmain_scanhash",given_id,job_id,given_id - job_id);
        }
        fclose((FILE *)pFile_4);
      }
...
LAB_000344bc:
      ...
      uVar3 = hashtest_submit(thr,work,nonce3,midstate,pool,
                              CONCAT44(nonce2._4_4_,(undefined4)nonce2),chain_id);
      h = uVar3 + h;
      _free_work(&work,"driver-btm-soc.c","bitmain_scanhash",0x2285);
    }
    if (given_id == job_id) {
      pool = pool_stratum0;
      goto LAB_000344bc;
    ...
    if (4 < log_level) {
      ...
      if (pFile_5 != (FILE *)0x0) {
        fprintf((FILE *)pFile_5,"%s:%d:%s: job_id non\'t found given=%d, job=%d...\n",
                "driver-btm-soc.c",0x227a,"bitmain_scanhash",given_id,job_id);
      }
      fclose((FILE *)pFile_5);
    }
    if (dev->chain_exist[chain_id] == 1) {
