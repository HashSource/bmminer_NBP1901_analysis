...

{
  ...
  if (allocate < 1) {
    snprintf(tmp42,0x800,"Invalid new list %s with allocate %d must be > 0 in %s %s():%d",name,
             allocate,"klist.c","_k_new_list",0x6a);
    _applog(3,tmp42,true);
    _quit(1);
  }
  if (limit < 0) {
    snprintf(tmp42,0x800,"Invalid new list %s with limit %d must be >= 0 in %s %s():%d",name,limit,
             "klist.c","_k_new_list",0x6d);
    _applog(3,tmp42,true);
    _quit(1);
  }
  list_00 = (K_LIST *)calloc(1,0x40);
  if (list_00 == (K_LIST *)0x0) {
    snprintf(tmp42,0x800,"Failed to calloc list %s in %s %s():%d",name,"klist.c","_k_new_list",0x71)
    ;
    _applog(3,tmp42,true);
    _quit(1);
  }
  ...
  _cglock_init(list_00->lock,"klist.c","_k_new_list",0x79);
  list_00->name = name;
  list_00->siz = siz;
