...

{
  ...
  for (_hj_k = strlen(cgpu->drv->name); 0xb < _hj_k; _hj_k = _hj_k - 0xc) {
    iVar10 = _hj_j + (uint)_hj_key[4] + (uint)_hj_key[5] * 0x100 + (uint)_hj_key[6] * 0x10000 +
    ...
    uVar6 = uVar5 >> 0xd ^
            ((_hj_i + (uint)*_hj_key + (uint)_hj_key[1] * 0x100 + (uint)_hj_key[2] * 0x10000 +
                      (uint)_hj_key[3] * 0x1000000) - iVar10) - uVar5;
  ...
  if (d == (_cgpu_devid_counter *)0x0) {
    ...
    for (; 0xb < _hj_k_1; _hj_k_1 = _hj_k_1 - 0xc) {
      iVar10 = _hj_j_1 + (uint)_hj_key_1[4] + (uint)_hj_key_1[5] * 0x100 +
      ...
      uVar6 = uVar5 >> 0xd ^
              ((_hj_i_1 +
               ...
               (uint)_hj_key_1[3] * 0x1000000) - iVar10) - uVar5;
      uVar7 = uVar6 << 8 ^ (iVar10 - uVar5) - uVar6;
      uVar5 = uVar7 >> 0xd ^ (uVar5 - uVar6) - uVar7;
      uVar6 = uVar5 >> 0xc ^ (uVar6 - uVar7) - uVar5;
      uVar7 = uVar6 << 0x10 ^ (uVar7 - uVar5) - uVar6;
  ...
  devices = (cgpu_info **)
            _cgrealloc(devices,(total_devices + new_devices + 2) * 4,"cgminer.c","add_cgpu",0x2bdc);
  ...
  if (hotplug_mode == false) {
    ...
    *ppcVar11 = cgpu;
    ...
    iVar10 = new_devices + total_devices;
