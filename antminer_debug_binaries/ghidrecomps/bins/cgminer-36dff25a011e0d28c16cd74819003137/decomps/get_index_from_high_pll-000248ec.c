...

{
  ...
  i = 0;
  while ((((uint)i < 0x21 && (freq_high_pll_1391[i].freq != freq)) &&
         ((freq_high_pll_1391[i].freq <= (uint)freq ||
          ((uint)freq <= freq_high_pll_1391[i + -1].freq))))) {
    i = i + 1;
  }
  if (i == 0x21) {
    if (1 < log_level) {
      ...
      if (__stream != (FILE *)0x0) {
        fprintf(__stream,"%s:%d:%s: high freq index set error,return default pll index\n",
                "driver-btm-soc.c",0x661,"get_index_from_high_pll");
      }
      fclose(__stream);
    }
