  ...
  if (doTestPatten == false) {
    if (*buf == 'R') {
      ...
      if (4 < log_level) {
        ...
        pFVar1 = fopen(log_file,"a+");
        if (pFVar1 != (FILE *)0x0) {
          fprintf(pFVar1,"%s:%d:%s: len = 0x%x\n","driver-btm-soc.c",0x1f52,"send_job",iVar3);
        }
        fclose(pFVar1);
      }
      __s = malloc(iVar3 + 8);
            ...
            if (4 < log_level) {
              ...
              if (pFVar1 != (FILE *)0x0) {
                fprintf(pFVar1,
                        ...
                        ,"driver-btm-soc.c",0x1f6c,"send_job",dev->current_job_start_address,
        ...
        if (__s_00 == (void *)0x0) {
          ...
          *(undefined4 *)((int)__s_00 + (coinbase_padding_len - 8)) = 0;
            ...
            if ((*(char *)((int)dev->current_job_start_address + i) != *(char *)((int)__s_00 + i))
              ...
              if (pFVar1 != (FILE *)0x0) {
                fprintf(pFVar1,
                        ...
                        ,"driver-btm-soc.c",0x1fb0,"send_job",i,
                        (uint)*(byte *)((int)dev->current_job_start_address + i),i,
              ...
              if ((*(uchar *)((int)dev->current_job_start_address + coinbase_padding_len + i) !=
                ...
                if (pFVar1 != (FILE *)0x0) {
                  fprintf(pFVar1,
                          ...
                          "driver-btm-soc.c",0x1fc5,"send_job",i,
          ...
          if (opt_multi_version == 2) {
            ...
            set_block_header_version_1(*(uint *)((int)__s + 0x10) | 0x4000);
            set_block_header_version_2(*(uint *)((int)__s + 0x10) | 0x8000);
            set_block_header_version_3(*(uint *)((int)__s + 0x10) | 0xc000);
          }
          else if ((opt_multi_version != 1) && (3 < log_level)) {
            print_crt_time_to_file(log_file,3);
            pFVar1 = fopen(log_file,"a+");
            if (pFVar1 != (FILE *)0x0) {
              fprintf(pFVar1,"%s:%d:%s: Note: Unknown opt_multi_version %d\n","driver-btm-soc.c",
                      0x1ffa,"send_job",opt_multi_version);
            }
            fclose(pFVar1);
          }
          memset(buf2,0,0x80);
          for (i = 0; i < 8; i = i + 1) {
            buf2[i] = (uint)*(byte *)((int)__s + i * 4 + 0x17) << 0x18 |
          ...
          set_coinbase_length_and_nonce2_length
                    ((uint)*(ushort *)((int)__s + 0x3e) << 0x10 |
                     (*(ushort *)((int)__s + 0x40) & 0xff) << 8 | uVar4 & 0xff);
          ...
          if ((*(byte *)((int)__s + 9) & 1) == 0) {
            if (pool->support_ab == false) {
              ...
              uVar4 = get_dhash_acc_control();
              set_dhash_acc_control(uVar4 & 0xfffff0bf | (opt_multi_version & 0xfU) << 8 | 0x8060);
            }
          }
          ...
          if (4 < log_level) {
            ...
            pFVar1 = fopen(log_file,"a+");
            if (pFVar1 != (FILE *)0x0) {
              fprintf(pFVar1,"%s:%d:%s: --- end\n","driver-btm-soc.c",0x2044,"send_job");
            }
            fclose(pFVar1);
          }
