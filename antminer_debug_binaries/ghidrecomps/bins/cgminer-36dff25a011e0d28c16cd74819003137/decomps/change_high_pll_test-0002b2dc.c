...
void change_high_pll_test(uint8_t chain,float freq,int index)

{
  ...
  if (opt_use_pll_list) {
    if (3 < log_level) {
      ...
      if (__stream != (FILE *)0x0) {
        fprintf(__stream,"%s:%d:%s: set freq %d, pll_out %d\n","driver-btm-soc.c",0x131c,
                "change_high_pll_test",freq_high_pll_1391[index].freq,pll_00);
      }
      fclose(__stream);
    }
  ...
  set_config_BM1391_t(chain,'\0','\x01','p',divider - 1 | 0xf0f0f00);
  set_config_BM1391_t(chain,'\0','\x01','\b',
