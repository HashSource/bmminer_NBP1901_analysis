  ...
  if (json == (json_t *)0x0) {
    ...
    if (((json_01 == (json_t *)0x0) || (json_01->type != JSON_NULL)) && (json_01 != (json_t *)0x0))
    {
      ...
      if (((pjVar5 == (json_t *)0x0) &&
        ...
        id = (int)jVar15;
        _mutex_lock(&sshare_lock,"cgminer.c","parse_stratum_response",0x1c5d);
        _hj_key = (uchar *)&id;
        _hf_hashv = 0xfeedbeef;
        _hj_j = 0x9e3779b9;
        _hj_i = 0x9e3779b9;
        for (_hj_k = 4; 0xb < _hj_k; _hj_k = _hj_k - 0xc) {
          iVar13 = _hj_j + (uint)_hj_key[4] + (uint)_hj_key[5] * 0x100 + (uint)_hj_key[6] * 0x10000
          ...
          uVar9 = uVar8 >> 0xd ^
                  ((_hj_i + (uint)*_hj_key + (uint)_hj_key[1] * 0x100 + (uint)_hj_key[2] * 0x10000 +
                            (uint)_hj_key[3] * 0x1000000) - iVar13) - uVar8;
        ...
        default:
          ...
          _mutex_unlock(&sshare_lock,"cgminer.c","parse_stratum_response",0x1c65);
          if (sshare == (stratum_share *)0x0) {
            if (json_00 != (json_t *)0x0) {
              _cg_rlock(&pool->data_lock,"cgminer.c","parse_stratum_response",0x1c71);
              pdVar12 = &pool->sdiff;
              dVar4 = *pdVar12;
              dVar3 = *pdVar12;
              dVar2 = *pdVar12;
              dVar1 = *pdVar12;
              _cg_runlock(&pool->data_lock,"cgminer.c","parse_stratum_response",0x1c73);
              if ((json_00 == (json_t *)0x0) || (json_00->type != JSON_TRUE)) {
                ...
                _mutex_lock(&stats_lock,"cgminer.c","parse_stratum_response",0x1c86);
                ...
                *(uint *)((int)&pool->rejected + 4) = iVar13 + (uint)(0xfffffffe < uVar8);
                ...
                _mutex_unlock(&stats_lock,"cgminer.c","parse_stratum_response",0x1c8b);
              }
              else {
                if (((use_syslog != false) || (opt_log_output != false)) || (4 < opt_log_level)) {
                ...
                _mutex_lock(&stats_lock,"cgminer.c","parse_stratum_response",0x1c7b);
                ...
                *(uint *)((int)&pool->accepted + 4) = iVar13 + (uint)(0xfffffffe < uVar8);
                ...
                _mutex_unlock(&stats_lock,"cgminer.c","parse_stratum_response",0x1c80);
              }
            }
          }
          else {
            stratum_share_result(json,json_00,err_val_00,sshare);
            _free_work(&sshare->work,"cgminer.c","parse_stratum_response",0x1c92);
            free(sshare);
            ret = true;
          }
        }
