...

{
  ...
  cleared = 0;
  _mutex_lock(&sshare_lock,"cgminer.c","prune_stratum_shares",0x2796);
  ...
  while (sshare != (stratum_share *)0x0) {
    if ((pool == sshare->work->pool) && (sshare->sshare_time + 0x78 < tVar1)) {
      if (((sshare->hh).prev == (void *)0x0) && ((sshare->hh).next == (void *)0x0)) {
        ...
        if (sshare == (stratum_share *)((stratum_shares->hh).tbl)->buckets[uVar3].hh_head) {
      ...
      _free_work(&sshare->work,"cgminer.c","prune_stratum_shares",0x279d);
      free(sshare);
      cleared = cleared + 1;
    }
    sshare = tmpshare;
  ...
  _mutex_unlock(&sshare_lock,"cgminer.c","prune_stratum_shares",0x27a3);
  if (cleared != 0) {
    ...
    bVar5 = CARRY4(cleared,(uint)total_stale);
    total_stale._0_4_ = cleared + (uint)total_stale;
    total_stale._4_4_ = (cleared >> 0x1f) + total_stale._4_4_ + (uint)bVar5;
  }
  return;
}
