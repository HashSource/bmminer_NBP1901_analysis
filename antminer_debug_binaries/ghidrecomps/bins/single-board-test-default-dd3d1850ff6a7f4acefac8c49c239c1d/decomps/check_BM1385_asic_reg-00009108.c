...

{
      ...
      for (local_10 = 0; local_10 < local_14; local_10 = local_10 + 1) {
        ...
        if (param_1 == *(byte *)(reg_value_buf + (*(int *)(reg_value_buf + 4) + 2) * 8 + 5)) {
          ...
          if (uVar1 == *(byte *)(reg_value_buf + (*(int *)(reg_value_buf + 4) + 2) * 8 + 4)) {
            if (param_3 == '\0') {
              cgpu[param_1 + 0x20338] = cgpu[param_1 + 0x20338] + '\x01';
            }
            if (param_3 == '\f') {
  ...
  local_c = 0;
  if (param_3 == '\0') {
    if ((uint)cgpu._131916_4_ < (uint)(byte)cgpu[param_1 + 0x20338]) {
      cgpu._131916_4_ = ZEXT14((byte)cgpu[param_1 + 0x20338]);
    }
    printf("%s: chain J%d has %d ASIC\n","check_BM1385_asic_reg",param_1 + 1,
           (uint)(byte)cgpu[param_1 + 0x20338]);
  }
  clear_register_value_buf();
  return;
}
