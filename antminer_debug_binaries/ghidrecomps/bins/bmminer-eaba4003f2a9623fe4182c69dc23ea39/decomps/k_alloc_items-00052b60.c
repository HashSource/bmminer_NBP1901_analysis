  ...
  if ((list->limit < 1) || (list->total < list->limit)) {
    ...
    if (pkVar2 == (k_item *)0x0) {
      snprintf(tmp42,0x800,
               ...
               list->name,allocate,list->total,list->limit,"klist.c","k_alloc_items",0x26);
      _applog(3,tmp42,true);
      _quit(1);
    }
    list->item_memory[list->item_mem_count + -1] = pkVar2;
    ...
    for (i = 1; i < allocate + -1; i = i + 1) {
      ...
      pkVar2[i].prev = pkVar2 + i + -1;
      pkVar2[i].next = pkVar2 + i + 1;
    }
    pkVar2[allocate + -1].name = list->name;
    pkVar2[allocate + -1].prev = pkVar2 + allocate + -2;
    pkVar2[allocate + -1].next = (k_item *)0x0;
    list->head = pkVar2;
    if (list->do_tail != false) {
      list->tail = pkVar2 + allocate + -1;
    }
    for (item = list->head; item != (K_ITEM *)0x0; item = item->next) {
      ...
      if (list->data_memory == (void **)0x0) {
        snprintf(tmp42,0x800,"List %s data_memory failed to realloc count=%d in %s %s():%d",
                 list->name,list->data_mem_count,"klist.c","k_alloc_items",0x45);
        _applog(3,tmp42,true);
        _quit(1);
      }
