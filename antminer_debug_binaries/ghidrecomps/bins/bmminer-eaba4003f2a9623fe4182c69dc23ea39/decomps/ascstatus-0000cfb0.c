...

{
  ...
  if ((((0 < numasc) && (-1 < asc_local)) && (asc_local < numasc)) &&
    ...
    cgpu->utility = ((double)(longlong)cgpu->accepted / dev_runtime) * DAT_0000d2d8;
    ...
    if ((uVar1 + *(uint *)&cgpu->diff1 |
        ((int)uVar1 >> 0x1f) +
        *(int *)((int)&cgpu->diff1 + 4) + (uint)CARRY4(uVar1,*(uint *)&cgpu->diff1)) != 0) {
      iVar3 = cgpu->hw_errors;
      uVar1 = cgpu->hw_errors;
      dVar2 = (double)__aeabi_l2d(uVar1 + *(uint *)&cgpu->diff1,
                                  ((int)uVar1 >> 0x1f) +
                                  *(int *)((int)&cgpu->diff1 + 4) +
                                  (uint)CARRY4(uVar1,*(uint *)&cgpu->diff1));
      hwp = (double)(longlong)iVar3 / dVar2;
    }
    root = api_add_percent(root,"Device Hardware%",&hwp,false);
    rejp = DAT_0000d2e0;
    if ((*(uint *)&cgpu->diff1 | *(uint *)((int)&cgpu->diff1 + 4)) != 0) {
      dVar4 = cgpu->diff_rejected;
      dVar2 = (double)__aeabi_l2d(*(undefined4 *)&cgpu->diff1,*(undefined4 *)((int)&cgpu->diff1 + 4)
                                 );
      rejp = dVar4 / dVar2;
    }
