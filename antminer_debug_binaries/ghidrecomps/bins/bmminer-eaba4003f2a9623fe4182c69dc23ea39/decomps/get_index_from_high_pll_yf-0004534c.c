...
int get_index_from_high_pll_yf(int freq)

{
  ...
  i = 0;
  while ((((uint)i < 0x26 && (freq_high_pll_1385[i].freq != freq)) &&
         ((freq_high_pll_1385[i].freq <= (uint)freq ||
          ((uint)freq <= freq_high_pll_1385[i + -1].freq))))) {
    i = i + 1;
  }
  if (i == 0x26) {
    ...
    i = get_index_from_high_pll_yf(200);
  }
  return i;
}
