...

{
  ...
  if ((pool->swork).job_id != (char *)0x0) {
    if ((pool_stratum->swork).job_id != (char *)0x0) {
      ...
      uVar1 = CRC16((uchar *)(pool->swork).job_id,(uint16_t)sVar3);
      sVar3 = strlen((pool_stratum->swork).job_id);
      uVar2 = CRC16((uchar *)(pool_stratum->swork).job_id,(uint16_t)sVar3);
      if (uVar1 == uVar2) {
        return;
      }
    }
    _cg_wlock(&pool_stratum->data_lock,"driver-btm-soc.c","copy_pool_stratum",0x2dab);
    free((pool_stratum->swork).job_id);
    free(pool_stratum->nonce1);
    free(pool_stratum->coinbase);
    puVar4 = (uchar *)_cgcalloc(__n,1,"driver-btm-soc.c","copy_pool_stratum",0x2db0);
    ...
    *(undefined4 *)((int)&pool_stratum->sdiff + 4) = uVar8;
    ...
    _cg_wunlock(&pool_stratum->data_lock,"driver-btm-soc.c","copy_pool_stratum",0x2dca);
  }
  return;
}
