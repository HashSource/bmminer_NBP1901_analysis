...

{
  ...
  rc = true;
  _mutex_lock(stgd_lock,"cgminer.c","hash_push",0x162f);
  ...
  if (getq->frozen == true) {
    ...
    for (_hj_k = 4; 0xb < _hj_k; _hj_k = _hj_k - 0xc) {
      ...
      uVar7 = uVar6 >> 0xd ^
              ((_hj_i + (uint)*_hj_key + (uint)_hj_key[1] * 0x100 + (uint)_hj_key[2] * 0x10000 +
                        (uint)_hj_key[3] * 0x1000000) - iVar5) - uVar6;
    ...
    uVar6 = uVar6 & ((staged_work->hh).tbl)->num_buckets - 1;
      ...
      for (_he_bkt_i = 0; _he_bkt_i < ((work->hh).tbl)->num_buckets; _he_bkt_i = _he_bkt_i + 1) {
        ...
        while (_he_thh != (UT_hash_handle *)0x0) {
          ...
          if (((work->hh).tbl)->ideal_chain_maxlen < pUVar10->count) {
            ...
            uVar6 = __aeabi_uidiv(pUVar10->count,((work->hh).tbl)->ideal_chain_maxlen);
        ...
        while (_hs_p != (UT_hash_handle *)0x0) {
          ...
          for (_hs_i = 0; _hs_i < _hs_insize; _hs_i = _hs_i + 1) {
            ...
            if (_hs_q->next == (void *)0x0) {
              ...
              _hs_q = (UT_hash_handle *)((int)_hs_q->next + ((staged_work->hh).tbl)->hho);
            }
            ...
            if (_hs_psize == 0) {
              ...
              if (_hs_p != (UT_hash_handle *)0x0) {
                if (_hs_p->next == (void *)0x0) {
                  ...
                  _hs_p = (UT_hash_handle *)((int)_hs_p->next + ((staged_work->hh).tbl)->hho);
                }
              }
              _hs_psize = _hs_psize - 1;
            }
  ...
  _mutex_unlock(stgd_lock,"cgminer.c","hash_push",0x1641);
  return rc;
}
