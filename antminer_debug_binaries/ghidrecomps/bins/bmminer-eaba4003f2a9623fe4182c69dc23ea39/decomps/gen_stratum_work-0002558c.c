...

{
  ...
  _cg_wlock(&pool->data_lock,"cgminer.c","gen_stratum_work",0x2035);
  nonce2le._0_4_ = *(undefined4 *)&pool->nonce2;
  nonce2le._4_4_ = *(undefined4 *)((int)&pool->nonce2 + 4);
  _cg_memcpy(pool->coinbase + pool->nonce2_offset,&nonce2le,pool->n2size,"cgminer.c",
             "gen_stratum_work",0x203a);
  ...
  *(uint *)((int)&pool->nonce2 + 4) = iVar4 + (uint)(0xfffffffe < uVar3);
  ...
  _cg_dwlock(&pool->data_lock,"cgminer.c","gen_stratum_work",0x2040);
  gen_hash(pool->coinbase,merkle_root,pool->coinbase_len);
  _cg_memcpy(merkle_sha,merkle_root,0x20,"cgminer.c","gen_stratum_work",0x2044);
  for (i = 0; i < pool->merkles; i = i + 1) {
    ...
    _cg_memcpy(merkle_sha,merkle_root,0x20,"cgminer.c","gen_stratum_work",0x204a);
  }
  flip32(merkle_root,merkle_sha);
  _cg_memcpy(work,pool->header_bin,0x70,"cgminer.c","gen_stratum_work",0x2053);
  pcVar6 = "gen_stratum_work";
  uVar7 = 0x2054;
  _cg_memcpy(work->data + 0x24,merkle_root,0x20,"cgminer.c","gen_stratum_work",0x2054);
  ...
  *(undefined4 *)((int)&work->sdiff + 4) = uVar5;
  ...
  work->ntime = pcVar1;
  _cg_runlock(&pool->data_lock,"cgminer.c","gen_stratum_work",0x205f);
  if (opt_debug != false) {
