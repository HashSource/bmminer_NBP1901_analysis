    ...
    while( true ) {
      ...
      if (sdiff.tv_sec < iVar6) {
        ...
        if (iVar6 < sdiff.tv_sec) {
          max_nonce = __aeabi_uidiv(max_nonce * iVar6,sdiff.tv_sec);
        }
          ...
          max_nonce = __aeabi_uidiv(max_nonce << 10,uVar4);
        }
LAB_00027b0a:
    ...
    dev->shutdown = true;
LAB_00027c54:
    _free_work(&work,"cgminer.c","hash_sole_work",0x2405);
  } while( true );
}
