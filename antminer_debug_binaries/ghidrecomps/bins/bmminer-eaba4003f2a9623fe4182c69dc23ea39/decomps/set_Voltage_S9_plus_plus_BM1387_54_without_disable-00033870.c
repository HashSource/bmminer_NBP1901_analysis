...
int set_Voltage_S9_plus_plus_BM1387_54_without_disable(uchar which_iic,uchar pic_voltage)
    ...
    while( true ) {
      ...
      if (((use_syslog != false) || (opt_log_output != false)) || (4 < opt_log_level)) {
        snprintf(tmp42,0x800,"%s failed on Chain[%d]!\n\n",
                 "set_Voltage_S9_plus_plus_BM1387_54_without_disable",(uint)which_iic);
        _applog(5,tmp42,false);
      }
      sleep(1);
    }
    printf("\n--- %s ok!\n\n","set_Voltage_S9_plus_plus_BM1387_54_without_disable",(uint)pic_voltage
          );
    cgsleep_ms(500);
    iVar4 = 1;
  }
