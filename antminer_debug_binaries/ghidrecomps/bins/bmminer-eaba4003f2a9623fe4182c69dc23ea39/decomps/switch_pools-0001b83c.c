...

{
  ...
  _cg_wlock(&control_lock,"cgminer.c","switch_pools",0x141e);
  pool_00 = currentpool;
  ...
  currentpool = ppVar2;
  _cg_wunlock(&control_lock,"cgminer.c","switch_pools",0x146f);
  ...
  _mutex_lock(&lp_lock,"cgminer.c","switch_pools",0x1477);
  pthread_cond_broadcast((pthread_cond_t *)&lp_cond);
  _mutex_unlock(&lp_lock,"cgminer.c","switch_pools",0x1479);
  return;
}
