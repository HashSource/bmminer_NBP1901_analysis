...

{
  ...
  dVar8 = (cgpu->total_mhashes / dVar8) * DAT_00018a60;
  uVar9 = __fixunsdfdi(SUB84(dVar8,0),(int)((ulonglong)dVar8 >> 0x20));
  dVar8 = cgpu->rolling * DAT_00018a60;
  ...
  sVar1 = strlen(buf);
  uVar2 = *(undefined4 *)&cgpu->diff_accepted;
  uVar4 = *(undefined4 *)((int)&cgpu->diff_accepted + 4);
  uVar5 = *(undefined4 *)&cgpu->diff_rejected;
  uVar6 = *(undefined4 *)((int)&cgpu->diff_rejected + 4);
  iVar7 = cgpu->hw_errors;
  snprintf(tmp13,0x40,"(%ds):%s (avg):%sh/s | A:%.0f R:%.0f HW:%d WU:%.1f/m",opt_log_interval,
           displayed_rolling,displayed_hashes,uVar2,uVar4,uVar5,uVar6,iVar7);
  sVar3 = strlen(tmp13);
  if (bufsiz <= sVar1 + sVar3) {
    snprintf(tmp42,0x800,"tailsprintf buffer overflow in %s %s line %d","cgminer.c","get_statline",
             0xbe1,uVar2,uVar4,uVar5,uVar6,iVar7);
